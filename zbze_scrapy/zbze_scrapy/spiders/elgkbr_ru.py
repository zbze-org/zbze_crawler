...
class ElgkbrRuSpider(CrawlSpider):
    name = 'elgkbr_ru'
    allowed_domains = ['elgkbr.ru']
    start_urls = ['https://elgkbr.ru/newskab']
    ...
    def __init__(self, *args, **kwargs):
        super(ElgkbrRuSpider, self).__init__(*args, **kwargs)
        self.dump_dir = f'../data/{self.name}'
        os.makedirs(self.dump_dir, exist_ok=True)

        self.file = open(os.path.join(self.dump_dir, 'elgkbr_ru.jsonl'), 'wb')
        self.exporter = JsonLinesItemExporter(self.file, encoding='utf-8', ensure_ascii=False)
        self.exporter.start_exporting()

        self.db = TinyDB(os.path.join(self.dump_dir, 'elgkbr_ru.json'))
        self.query = Query()
    ...
    def parse_item(self, response):
        category = response.css('div.field-name-field-rubrika-kab a::attr(href)').get()
        if category != '/newskab':
            return
